// --- server.js (v28.2 - Fixed Dialer Code Verification State Bug, Updated Codes, Cleaned) ---
...
const dropCodeMap = {
  '0000': '15425', // Updated to numeric string based on client log
  '1234': '98765', // Updated to numeric string example
};
        ...
        socket.on('check_drop_id', ({ dropId }) => {
             ...
             if (requiresCode && targetOwnerSocketId && targetOwnerData) {
                  if (targetOwnerData.callState !== 'idle') {
                     ...
                     // *** FIX APPLIED HERE: Transition server state immediately ***
                 ...
                 updateUserState(socket.id, { callState: 'idle', targetDropId: null });
                  ...
                  console.log(`[Dialer] Public Drop ID ${dropId} found.`);
                     socket.emit('drop_id_status', {
                         ...
                         targetSocketId: 'SOCKET_ID_FOR_PUBLIC_LINE'
                     });
             }
             else {
